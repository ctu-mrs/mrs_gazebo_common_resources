 ...
 * Copyright (C) 2025 Petr Stibinger, Fly4Future
 *
...
#include "mrs_gazebo_common_resources/mrs_multicopter_motor_model.hpp"
...
public:
  FirstOrderFilter(double _timeConstantUp, double _timeConstantDown, T _initialState)
      :  // NOLINT
...
class gz::sim::systems::MrsMulticopterMotorModelPrivate {
...
public:
  void UpdateForcesAndMoments(EntityComponentManager &_ecm);
...
public:
  int actuatorNumber = 0;

  /// \brief Turning direction of the motor.
public:
  int turningDirection = turning_direction::kCw;
...
public:
  double maxRotVelocity = 838.0;

  /// \brief Moment constant for computing drag torque based on thrust
  /// with units of length (m).
  /// The default value is taken from gazebo_motor_model.h
public:
  double momentConstant = 0.016;

  /// \brief Thrust coefficient for propeller with units of N / (rad/s)^2.
  /// The default value is taken from gazebo_motor_model.h
public:
  double motorConstant = 8.54858e-06;

  /// \brief Reference input to motor.
  /// This is the throttle signal mapped to range 0 - 1000
public:
  ...
  /// The default value is taken from gazebo_motor_model.h
public:
  double rollingMomentCoefficient = 1.0e-6;

  /// \brief Rotor drag coefficient for propeller with units of N / (m/s^2).
  /// The default value is taken from gazebo_motor_model.h
public:
  double rotorDragCoefficient = 1.0e-4;
  ...
  /// The default value is taken from gazebo_motor_model.h
public:
  double rotorVelocitySlowdownSim = 10.0;

  /// \brief Time constant for rotor deceleration.
  /// The default value is taken from gazebo_motor_model.h
public:
  double timeConstantDown = 1.0 / 40.0;

  /// \brief Time constant for rotor acceleration.
  /// The default value is taken from gazebo_motor_model.h
public:
  double timeConstantUp = 1.0 / 80.0;

  /* public: */
  /*   double throttleMappingQuadratic = -257.22262657; */
  /*   double throttleMappingLinear    = 1054.25640837; */
  /*   double throttleMappingConstant  = -26.0; */

  /* public: */
  /*   double rotToForceQuadratic = 0.0000396767562; */
  /*   double rotToForceLinear    = -0.00323878661; */
  /*   double rotToForceConstant  = 0.20109244; */

public:
  double minThrottle = 0.1;


  /// \brief Filter on rotor velocity that has different time constants
  /// for increasing and decreasing values.
public:
  std::unique_ptr<FirstOrderFilter<double>> rotorVelocityFilter;

  /// \brief Received Actuators message. This is nullopt if no message has been
  /// received.
public:
  std::optional<msgs::Actuators> recvdActuatorsMsg;

  /// \brief Mutex to protect recvdActuatorsMsg.
public:
  std::mutex recvdActuatorsMsgMutex;
...
//////////////////////////////////////////////////
MrsMulticopterMotorModel::MrsMulticopterMotorModel() : dataPtr(std::make_unique<MrsMulticopterMotorModelPrivate>()) {
}

//////////////////////////////////////////////////
void MrsMulticopterMotorModel::Configure(const Entity &_entity, const std::shared_ptr<const sdf::Element> &_sdf, EntityComponentManager &_ecm, EventManager & /*_eventMgr*/) {
  this->dataPtr->model = Model(_entity);

  if (!this->dataPtr->model.Valid(_ecm)) {
    gzerr << "MrsMulticopterMotorModel plugin should be attached to a model " << "entity. Failed to initialize." << std::endl;
    return;
  }
  ...
  if (sdfClone->HasElement("robotNamespace")) {
    this->dataPtr->robotNamespace = sdfClone->Get<std::string>("robotNamespace");
  } else {
  ...
  if (this->dataPtr->jointName.empty()) {
    gzerr << "MrsMulticopterMotorModel found an empty jointName parameter. " << "Failed to initialize.";
    return;
  }
  ...
  if (this->dataPtr->linkName.empty()) {
    gzerr << "MrsMulticopterMotorModel found an empty linkName parameter. " << "Failed to initialize.";
    return;
  }

  if (sdfClone->HasElement("actuator_number")) {
    this->dataPtr->actuatorNumber = sdfClone->GetElement("actuator_number")->Get<int>();
  } else if (sdfClone->HasElement("motorNumber")) {
    this->dataPtr->actuatorNumber = sdfClone->GetElement("motorNumber")->Get<int>();
    gzwarn << "<motorNumber> is depricated, " << "please use <actuator_number>.\n";
  } else {
    gzerr << "Please specify a actuator_number.\n";
  }

  if (sdfClone->HasElement("turningDirection")) {
    auto turningDirection = sdfClone->GetElement("turningDirection")->Get<std::string>();
  ...
  sdfClone->Get<std::string>("commandSubTopic", this->dataPtr->commandSubTopic, this->dataPtr->commandSubTopic);

  sdfClone->Get<double>("rotorDragCoefficient", this->dataPtr->rotorDragCoefficient, this->dataPtr->rotorDragCoefficient);
  sdfClone->Get<double>("rollingMomentCoefficient", this->dataPtr->rollingMomentCoefficient, this->dataPtr->rollingMomentCoefficient);
  ...
  sdfClone->Get<double>("timeConstantUp", this->dataPtr->timeConstantUp, this->dataPtr->timeConstantUp);
  sdfClone->Get<double>("timeConstantDown", this->dataPtr->timeConstantDown, this->dataPtr->timeConstantDown);
  sdfClone->Get<double>("rotorVelocitySlowdownSim", this->dataPtr->rotorVelocitySlowdownSim, 10);

  // Create the first order filter.
  this->dataPtr->rotorVelocityFilter = std::make_unique<FirstOrderFilter<double>>(this->dataPtr->timeConstantUp, this->dataPtr->timeConstantDown, 0.0);
  ...
  if (topic.empty()) {
    gzerr << "Failed to create topic for [" << this->dataPtr->robotNamespace << "]" << std::endl;
    return;
  } else {
    gzdbg << "Listening to topic: " << topic << std::endl;
  }
  this->dataPtr->node.Subscribe(topic, &MrsMulticopterMotorModelPrivate::OnActuatorMsg, this->dataPtr.get());
}

//////////////////////////////////////////////////
void MrsMulticopterMotorModel::PreUpdate(const UpdateInfo &_info, EntityComponentManager &_ecm) {
  GZ_PROFILE("MrsMulticopterMotorModel::PreUpdate");
  ...
  if (this->dataPtr->jointEntity == kNullEntity) {
    this->dataPtr->jointEntity = this->dataPtr->model.JointByName(_ecm, this->dataPtr->jointName);

    const auto parentLinkName = _ecm.Component<components::ParentLinkName>(this->dataPtr->jointEntity);
    if (parentLinkName)
      this->dataPtr->parentLinkName = parentLinkName->Data();
  }

  if (this->dataPtr->linkEntity == kNullEntity) {
    this->dataPtr->linkEntity = this->dataPtr->model.LinkByName(_ecm, this->dataPtr->linkName);
  }
  ...
  if (this->dataPtr->jointEntity == kNullEntity || this->dataPtr->linkEntity == kNullEntity || this->dataPtr->parentLinkEntity == kNullEntity)
    return;

  // skip UpdateForcesAndMoments if needed components are missing
  bool doUpdateForcesAndMoments = true;

  const auto jointVelocity = _ecm.Component<components::JointVelocity>(this->dataPtr->jointEntity);
  if (!jointVelocity) {
...
//////////////////////////////////////////////////
void MrsMulticopterMotorModelPrivate::OnActuatorMsg(const msgs::Actuators &_msg) {
  std::lock_guard<std::mutex> lock(this->recvdActuatorsMsgMutex);
  this->recvdActuatorsMsg = _msg;
}

//////////////////////////////////////////////////
void MrsMulticopterMotorModelPrivate::UpdateForcesAndMoments(EntityComponentManager &_ecm) {
  GZ_PROFILE("MrsMulticopterMotorModelPrivate::UpdateForcesAndMoments");
  ...
  if (msg.has_value()) {
    if (this->actuatorNumber > msg->velocity_size() - 1) {
      gzerr << "You tried to access index " << this->actuatorNumber << " of the Actuator velocity array which is of size " << msg->velocity_size() << std::endl;
      return;
    }
    double throttle_input = msg->velocity(this->actuatorNumber) / 1000.0;
    if (throttle_input <= 0)
      throttle_input = 0;
    else if (throttle_input <= this->minThrottle) {
      throttle_input = this->minThrottle;
    }
    this->throttleInput = throttle_input;
  }

  const auto jointVelocity = _ecm.Component<components::JointVelocity>(this->jointEntity);
  ...
  double thrust = this->turningDirection * realMotorVelocitySign * realMotorVelocity * realMotorVelocity * this->motorConstant;
  /* double thrust = this->turningDirection * realMotorVelocitySign * ((realMotorVelocity * realMotorVelocity * this->rotToForceQuadratic) + (realMotorVelocity * this->rotToForceLinear) + this->rotToForceConstant); */

  /* gzwarn << "Throttle input: " << this->throttleInput << std::endl; */
  /* gzwarn << "RealMotorVelocity: " << realMotorVelocity << " rad/s" << std::endl; */
  /* gzwarn << "Applied thrust: " << thrust << " N" << std::endl; */
  ...
  const auto jointPose = _ecm.Component<components::Pose>(this->jointEntity);
  if (!jointPose) {
    gzerr << "joint " << this->jointName << " has no Pose" << "component" << std::endl;
    return;
  }
  ...
  const auto jointAxisComp = _ecm.Component<components::JointAxis>(this->jointEntity);
  if (!jointAxisComp) {
    gzerr << "joint " << this->jointName << " has no JointAxis" << "component" << std::endl;
    return;
  }
  ...
  Vector3 bodyVelocityPerpendicular = relativeWindVelocityWorld - (relativeWindVelocityWorld.Dot(jointAxis) * jointAxis);
  Vector3 airDrag                   = -std::abs(realMotorVelocity) * this->rotorDragCoefficient * bodyVelocityPerpendicular;
  ...
  auto    parentWrenchComp = _ecm.Component<components::ExternalWorldWrenchCmd>(this->parentLinkEntity);
  ...
  Vector3 dragTorque(0, 0, -this->turningDirection * thrust * this->momentConstant);
  // Transforming the drag torque into the parent frame to handle
  // arbitrary rotor orientations.
  Vector3 dragTorqueParentFrame = poseDifference.Rot().RotateVector(dragTorque);
  parentWorldTorque             = parentWorldPose->Rot().RotateVector(dragTorqueParentFrame);

  Vector3 rollingMoment;
  // - \omega * \mu_1 * V_A^{\perp}
  rollingMoment = -std::abs(realMotorVelocity) * this->rollingMomentCoefficient * bodyVelocityPerpendicular;
  ...
  if (!parentWrenchComp) {
    ...
    msgs::Set(parentWrenchComp->Data().mutable_torque(), msgs::Convert(parentWrenchComp->Data().torque()) + parentWorldTorque);
  }

  double refMotorRotVel = this->throttleInput * this->maxRotVelocity;

  /* double refMotorRotVel; */
  /* if (this->throttleInput < this->minThrottleToSpin) { */
  /*   refMotorRotVel = 0.0; */
  /* } else { */
  /*   refMotorRotVel = this->throttleMappingQuadratic * this->throttleInput * this->throttleInput + this->throttleMappingLinear * this->throttleInput + this->throttleMappingConstant; */
  /* } */

  // Apply the filter on the motor's velocity.
  refMotorRotVel = this->rotorVelocityFilter->UpdateFilter(refMotorRotVel, this->samplingTime);

  const auto jointVelCmd = _ecm.Component<components::JointVelocityCmd>(this->jointEntity);
  *jointVelCmd           = components::JointVelocityCmd({this->turningDirection * refMotorRotVel / this->rotorVelocitySlowdownSim});
}

GZ_ADD_PLUGIN(MrsMulticopterMotorModel, System, MrsMulticopterMotorModel::ISystemConfigure, MrsMulticopterMotorModel::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(MrsMulticopterMotorModel, "gz::sim::systems::MrsMulticopterMotorModel")
