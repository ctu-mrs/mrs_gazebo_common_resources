...
namespace gazebo
{
  class GroundGimbaledROSCamera : public CameraPlugin, GazeboRosCameraUtils
  {

  public:
    ////////////////////////////////////////////////////////////////////////////////
    // Constructor
    /* GroundGimbaledROSCamera() constructor //{ */
    GroundGimbaledROSCamera()
    {
    }
    //}
    ...
    ////////////////////////////////////////////////////////////////////////////////
    // Destructor
    /* ~GroundGimbaledROSCamera() destructor //{ */
    ~GroundGimbaledROSCamera()
    {
      ROS_DEBUG_STREAM_NAMED("ground gimbaled camera", "Unloaded");
    }
    //}
    ...
    void Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
    {
      ...
      {
        ROS_FATAL_STREAM_NAMED("ground gimbaled camera", "A ROS node for Gazebo has not been initialized, unable to load plugin. "
      ...
      {
        ROS_INFO_NAMED("ground gimbaled camera", "camera plugin missing <parentFrameName>, defaults to \"world\"");
      ...
      {
        ROS_INFO_NAMED("ground gimbaled camera", "camera plugin missing <sensorBaseFrameName>, defaults to \"sensor_base\"");
      ...
      {
        ROS_INFO_NAMED("ground gimbaled camera", "camera plugin missing <x>, defaults to 0");
        this->x_ = 0;
      } else
      ...
      {
        ROS_INFO_NAMED("ground gimbaled camera", "camera plugin missing <y>, defaults to 0");
        this->y_ = 0;
      } else
      ...
      {
        ROS_INFO_NAMED("ground gimbaled camera", "camera plugin missing <z>, defaults to 0");
        this->z_ = 0;
      } else
      ...
      {
        ROS_INFO_NAMED("ground gimbaled camera", "camera plugin missing <roll>, defaults to 0");
        this->roll_ = 0;
      } else
      ...
      {
        ROS_INFO_NAMED("ground gimbaled camera", "camera plugin missing <pitch>, defaults to 0");
        this->pitch_ = 0;
      } else
      ...
      {
        ROS_INFO_NAMED("ground gimbaled camera", "camera plugin missing <yaw>, defaults to 0");
        this->yaw_ = 0;
      } else
        this->yaw_ = _sdf->Get<double>("yaw");

      if (!_sdf->HasElement("targetName"))
      {
        ROS_INFO_NAMED("ground gimbaled camera", "camera plugin missing <targetName>, defaults to \"uav\"");
        this->base_frame_name_ = "uav";
      } else
        this->base_frame_name_ = _sdf->Get<std::string>("targetName");
    
      this->load_thread_ = boost::thread(boost::bind(&GroundGimbaledROSCamera::transformThread, this));
    }
    //}
  ...
  private:
    ...
    void transformThread()
    {
      ...
      ROS_INFO_NAMED("ground gimbaled camera", "camera plugin - GazeboRosCameraUtils initialized");
      this->tf_pub_ = this->rosnode_->advertise<tf2_msgs::TFMessage>("/tf_gazebo_static", 10, false);
    
      createStaticTransforms();
      this->timer_ = this->rosnode_->createWallTimer(ros::WallDuration(1.0), &GroundGimbaledROSCamera::publishStaticTransforms, this);
    }
    //}
  ...
  GZ_REGISTER_SENSOR_PLUGIN(GroundGimbaledROSCamera)
