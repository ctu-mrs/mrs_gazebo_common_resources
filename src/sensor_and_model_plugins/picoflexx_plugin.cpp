...
#define DEPTH_CAMERA_TOPIC "tof_depth"  // changed from depth_to_color_aligned
...
const std::string DEPTH_CAMERA_SUFFIX = "tof_depth";
// const std::string COLOR_CAMERA_SUFFIX = "color";
// const std::string IREDS_CAMERA_SUFFIX = "infra_stereo";
const std::string IRED_CAMERA_SUFFIX = "infra";
// const std::string IRED2_CAMERA_SUFFIX = "infra2";
const std::string BASE_FRAME_SUFFIX   = "link";

/// \brief A plugin that simulates PMD Picoflexx2 camera streams.
/* class PicoFlexxPlugin : public ModelPlugin //{ */
...
public:
  ...
  virtual void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
    ...
    std::cout << "Loading PicoFlexxPlugin with namespace " << camera_namespace << std::endl;

    // Store a pointer to the world
    this->world = this->pfModel->GetWorld();

    // Sensors Manager
    sensors::SensorManager* smanager = sensors::SensorManager::Instance();

    std::string camera_name = "picoflexx";
    if (!_sdf->HasElement("camera_name")) {
      ROS_INFO_NAMED("picoflexx", "picoflexx plugin missing <camera_name>, defaults to \"%s\"", camera_name.c_str());
    } else {
    ...
    if (!_sdf->HasElement("camera_suffix")) {
      ROS_INFO_NAMED("picoflexx", "picoflexx plugin missing <camera_suffix>, defaults to \"%s\"", camera_suffix.c_str());
    } else {
    ...
    ired_camera_plugin_name_ = camera_namespace + "/" + camera_name + camera_suffix + "_" + IRED_CAMERA_SUFFIX;
    ...
    const auto iredPtr = smanager->GetSensor(ired_camera_plugin_name_);
    // const auto iredsPtr = smanager->GetSensor(ireds_camera_plugin_name_);

    // Check if camera renderers have been found successfuly
    if (!depthPtr) {
      gzerr << "PicoFlexxPlugin: Depth Camera with name \"" << depth_camera_plugin_name_ << "\" has not been found" << std::endl;
      return;
    }
    if (!iredPtr) {
      gzerr << "PicoFlexxPlugin: Infrared Camera with name \"" << ired_camera_plugin_name_ << "\" has not been found" << std::endl;
      return;
    }
    // if (!iredsPtr) {
    //   gzerr << "PicoFlexxPlugin: InfraRed Stereo Camera with name \"" << ireds_camera_plugin_name_ << "\" has not been found" << std::endl;
    //   return;
    // }

    this->depthCam      = std::dynamic_pointer_cast<sensors::DepthCameraSensor>(depthPtr)->DepthCamera();
    this->iredCam       = std::dynamic_pointer_cast<sensors::CameraSensor>(iredPtr)->Camera();
    ...
    try {
      ...
      gzerr << "PicoFlexxPlugin: depthMap allocation failed: " << e.what() << std::endl;
      return;
    }
    ...
    if (!_sdf->HasElement("camera_name")) {
      gzwarn << "PicoFlexxPlugin: cannot load the camera_name parameter" << std::endl;
    } else {
      camera_name = _sdf->Get<std::string>("camera_name");
    }

    if (!_sdf->HasElement("camera_suffix")) {
      gzwarn << "PicoFlexxPlugin: cannot load the camera_suffix parameter" << std::endl;
    } else {
      camera_suffix = _sdf->Get<std::string>("camera_suffix");
    }
    ...
    std::string rsTopicRoot =
        /* "~/rs_d435/"; */
        "~/" + this->pfModel->GetName() + "/rs/stream/";
    ...
    this->iredPub = this->transportNode->Advertise<msgs::ImageStamped>(rsTopicRoot + "/" + IRED_CAMERA_TOPIC, 1, IRED_PUB_FREQ_HZ);
    ...
    if (this->useRealistic) {
      this->newDepthFrameConn =
          this->depthCam->ConnectNewDepthFrame(std::bind(&PicoFlexxPlugin::OnNewDepthFrameRealistic, this, this->depthCam, this->depthPub));
    } else {
      this->newDepthFrameConn = this->depthCam->ConnectNewDepthFrame(std::bind(&PicoFlexxPlugin::OnNewDepthFrame, this, this->depthCam, this->depthPub));
    }
    ...
    for (unsigned int i = 0; i < this->iredStereoCam->CameraCount(); ++i) {
      ...
      if (cameraName.find(ired1_camera_plugin_name_) != std::string::npos) {
        ...
        this->newIred1FrameConn = this->iredCams[i]->ConnectNewImageFrame(std::bind(&PicoFlexxPlugin::OnNewFrame, this, this->iredCams[i], this->ired1Pub));
      } else if (cameraName.find(ired2_camera_plugin_name_) != std::string::npos) {
        std::cout << "ired2 found" << std::endl;
        this->newIred2FrameConn = this->iredCams[i]->ConnectNewImageFrame(std::bind(&PicoFlexxPlugin::OnNewFrame, this, this->iredCams[i], this->ired2Pub));
      }
    }
    this->iredStereoCam->SetActive(true);
    */

    // this->newColorFrameConn = this->colorCam->ConnectNewImageFrame(std::bind(&PicoFlexxPlugin::OnNewFrame, this, this->colorCam, this->colorPub));
    this->newIredFrameConn = this->iredCam->ConnectNewImageFrame(std::bind(&PicoFlexxPlugin::OnNewFrame, this, this->iredCam, this->iredPub));

    // Listen to the update event
    this->updateConnection = event::Events::ConnectWorldUpdateBegin(boost::bind(&PicoFlexxPlugin::OnUpdate, this));
  }
...
protected:
  ...
  std::string ired_camera_plugin_name_;
...
protected:
  ...
  // / \brief Pointer to the IR Camera Renderer.
  ...
  /// \brief Pointer to the IR Publisher.
  ...
  /// \brief Pointer to the IR Camera callback connection.
...
/// \brief A plugin that simulates PicoFlexx camera streams.
/* class GazeboRosPicoflexx : public PicoFlexxPlugin //{ */

class GazeboRosPicoflexx : public PicoFlexxPlugin {
public:
  /////////////////////////////////////////////////
  /* GazeboRosPicoflexx() constructor //{ */
  GazeboRosPicoflexx() {
  }
  //}

  /////////////////////////////////////////////////
  /* ~GazeboRosPicoflexx() destructor //{ */
  ~GazeboRosPicoflexx() {
    ROS_DEBUG_STREAM_NAMED("picoflexx_camera", "Unloaded");
  }
  //}
  ...
  virtual void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
    ...
    ROS_INFO("Picoflexx Gazebo ROS plugin loading...");
    PicoFlexxPlugin::Load(_model, _sdf);

    cameraNamespace = _model->GetName();
    ROS_INFO_STREAM("Picoflexx camera attached to " << cameraNamespace);

    if (!_sdf->HasElement("camera_name")) {
      ROS_INFO_NAMED("picoflexx", "picoflexx plugin missing <camera_name>, defaults to \"\"");
      this->picoflexx_namespace = "";
    } else {
      this->picoflexx_namespace = _sdf->Get<std::string>("camera_name");
      camera_name_               = this->picoflexx_namespace;
    }

    if (!_sdf->HasElement("camera_suffix")) {
      ROS_INFO_NAMED("picoflexx", "picoflexx plugin missing <camera_suffix>, defaults to \"\"");
      this->picoflexx_namespace = "";
    } else {
      this->picoflexx_namespace = _sdf->Get<std::string>("camera_suffix");
      camera_suffix_             = this->picoflexx_namespace;
    }

    depth_camera_frame_id_  = cameraNamespace + "/" + camera_name_ + camera_suffix_ + "/" + DEPTH_CAMERA_SUFFIX;
    ired_camera_frame_id_   = cameraNamespace + "/" + camera_name_ + camera_suffix_ + "/" + IRED_CAMERA_SUFFIX;
    ...
    ired_camera_optical_frame_id_   = ired_camera_frame_id_ + "_optical";
    ...
    if (!_sdf->HasElement("parentFrameName")) {
      ROS_INFO_NAMED("picoflexx", "picoflexx plugin missing <parentFrameName>, defaults to \"world\"");
      this->parent_frame_name_ = "world";
    } else
      this->parent_frame_name_ = _sdf->Get<std::string>("parentFrameName");

    if (!_sdf->HasElement("x")) {
      ROS_INFO_NAMED("picoflexx", "picoflexx plugin missing <x>, defaults to 0");
      this->x_ = 0;
    } else
      this->x_ = _sdf->Get<double>("x");

    if (!_sdf->HasElement("y")) {
      ROS_INFO_NAMED("picoflexx", "picoflexx plugin missing <y>, defaults to 0");
      this->y_ = 0;
    } else
      this->y_ = _sdf->Get<double>("y");

    if (!_sdf->HasElement("z")) {
      ROS_INFO_NAMED("picoflexx", "picoflexx plugin missing <z>, defaults to 0");
      this->z_ = 0;
    } else
      this->z_ = _sdf->Get<double>("z");

    if (!_sdf->HasElement("roll")) {
      ROS_INFO_NAMED("picoflexx", "picoflexx plugin missing <roll>, defaults to 0");
      this->roll_ = 0;
    } else
      this->roll_ = _sdf->Get<double>("roll");

    if (!_sdf->HasElement("pitch")) {
      ROS_INFO_NAMED("picoflexx", "picoflexx plugin missing <pitch>, defaults to 0");
      this->pitch_ = 0;
    } else
      this->pitch_ = _sdf->Get<double>("pitch");

    if (!_sdf->HasElement("yaw")) {
      ROS_INFO_NAMED("picoflexx", "picoflexx plugin missing <yaw>, defaults to 0");
      this->yaw_ = 0;
    } else
    ...
    this->timer_ = this->rosnode_.createWallTimer(ros::WallDuration(1.0), &GazeboRosPicoflexx::publishStaticTransforms, this);
  }
  ...
  void createStaticTransforms() {
    ...
    geometry_msgs::TransformStamped static_transform_ired_base;
    ...
    geometry_msgs::TransformStamped static_transform_iredcam_infra;
    ...
    static_transform_depth_base.transform.translation.x = 0.0386;
    static_transform_depth_base.transform.translation.y = 0;
    ...
    static_transform_ired_base.header.stamp    = static_transform_base_world.header.stamp;
    static_transform_ired_base.header.frame_id = base_frame_id_;
    static_transform_ired_base.child_frame_id  = ired_camera_frame_id_;

    static_transform_ired_base.transform.translation.x = 0.0386;
    static_transform_ired_base.transform.translation.y = 0;
    static_transform_ired_base.transform.translation.z = 0;
    static_transform_ired_base.transform.rotation      = static_transform_depth_base.transform.rotation;
    ...
    static_transform_iredcam_infra.header.stamp    = static_transform_base_world.header.stamp;
    static_transform_iredcam_infra.header.frame_id = ired_camera_frame_id_;
    static_transform_iredcam_infra.child_frame_id  = ired_camera_optical_frame_id_;

    static_transform_iredcam_infra.transform.rotation = static_transform_depthcam_depth.transform.rotation;
    ...
    this->tf_message_.transforms.push_back(static_transform_ired_base);
    ...
    this->tf_message_.transforms.push_back(static_transform_iredcam_infra);
  ...
  virtual void OnNewFrame(const rendering::CameraPtr cam, [[maybe_unused]] const transport::PublisherPtr pub) {
    ...
    if (camera_id.find(ired_camera_plugin_name_) != std::string::npos) {
      camera_id = ired_camera_optical_frame_id_;
      image_pub = &(this->ired_pub_);
      ...
      ROS_ERROR("[GazeboRosPicoflexx]: Unknown camera name: %s\n", camera_id.c_str());
      camera_id = ired_camera_optical_frame_id_;
      image_pub = &(this->ired_pub_);
    }
  ...
  virtual void OnNewDepthFrame(const rendering::CameraPtr cam, const transport::PublisherPtr pub) {
    ...
    PicoFlexxPlugin::OnNewDepthFrame(cam, pub);
  ...
  virtual void OnNewDepthFrameRealistic(const rendering::CameraPtr cam, const transport::PublisherPtr pub) {
    ...
    PicoFlexxPlugin::OnNewDepthFrameRealistic(cam, pub);
...
private:
  ...
  std::string camera_name_    = "picoflexx";
...
protected:
  // image_transport::CameraPublisher color_pub_, ired_pub_, ir1_pub_, ir2_pub_, depth_pub_;
  image_transport::CameraPublisher ired_pub_, depth_pub_;
...
protected:
  ...
  std::string ired_camera_optical_frame_id_;
...
// Register the plugin
GZ_REGISTER_MODEL_PLUGIN(GazeboRosPicoflexx)
}  // namespace gazebo
