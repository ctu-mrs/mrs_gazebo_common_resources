...
#include <ignition/math/Vector3.hh>
#include <mrs_gazebo_common_resources/LivoxCustomMsg.h>
#include <limits>
...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
  ...
  std::string package_path = ros::package::getPath("mrs_gazebo_common_resources");
  std::filesystem::path file_path = std::filesystem::path(package_path) / "config" / "livox_scan_mode" / file_name;
  file_name = file_path.string();

  ROS_INFO_STREAM("loading csv file name:" << file_name);
  ...
  if (!sdf->HasElement("parentFrameName")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <parentFrameName>, defaults to \"fcu\"");
    parent_frame_name_ = "fcu";
  } else {
    parent_frame_name_ = sdf->Get<std::string>("parentFrameName");
  }

  if (!sdf->HasElement("sensorFrameName")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <sensorFrameName>, defaults to \"sensor\"");
    sensor_frame_name_ = "sensor";
  } else {
    sensor_frame_name_ = sdf->Get<std::string>("sensorFrameName");
  }

  if (!sdf->HasElement("lidarFrameName")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <lidarFrameName>, defaults to \"lidar\"");
    lidar_frame_name_ = "lidar";
  } else {
    lidar_frame_name_ = sdf->Get<std::string>("lidarFrameName");
  }

  if (!sdf->HasElement("sensor_x")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <sensor_x>, defaults to 0");
    sensor_x = 0;
  } else
    sensor_x = sdf->Get<double>("sensor_x");

  if (!sdf->HasElement("sensor_y")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <sensor_y>, defaults to 0");
    sensor_y = 0;
  } else
    sensor_y = sdf->Get<double>("sensor_y");

  if (!sdf->HasElement("sensor_z")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <sensor_z>, defaults to 0");
    sensor_z = 0;
  } else
    sensor_z = sdf->Get<double>("sensor_z");

  if (!sdf->HasElement("sensor_roll")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <sensor_roll>, defaults to 0");
    sensor_roll = 0;
  } else
    sensor_roll = sdf->Get<double>("sensor_roll");

  if (!sdf->HasElement("sensor_pitch")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <sensor_pitch>, defaults to 0");
    sensor_pitch = 0;
  } else
    sensor_pitch = sdf->Get<double>("sensor_pitch");

  if (!sdf->HasElement("sensor_yaw")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <sensor_yaw>, defaults to 0");
    sensor_yaw = 0;
  } else
    sensor_yaw = sdf->Get<double>("sensor_yaw");

  if (!sdf->HasElement("lidar_x")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <lidar_x>, defaults to 0");
    lidar_x = 0;
  } else
    lidar_x = sdf->Get<double>("lidar_x");

  if (!sdf->HasElement("lidar_y")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <lidar_y>, defaults to 0");
    lidar_y = 0;
  } else
    lidar_y = sdf->Get<double>("lidar_y");

  if (!sdf->HasElement("lidar_z")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <lidar_z>, defaults to 0");
    lidar_z = 0;
  } else
    lidar_z = sdf->Get<double>("lidar_z");

  if (!sdf->HasElement("lidar_roll")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <lidar_roll>, defaults to 0");
    lidar_roll = 0;
  } else
    lidar_roll = sdf->Get<double>("lidar_roll");

  if (!sdf->HasElement("lidar_pitch")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <lidar_pitch>, defaults to 0");
    lidar_pitch = 0;
  } else
    lidar_pitch = sdf->Get<double>("lidar_pitch");

  if (!sdf->HasElement("lidar_yaw")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <lidar_yaw>, defaults to 0");
    lidar_yaw = 0;
  } else {
    lidar_yaw = sdf->Get<double>("lidar_yaw");
  }

  //}

  std::string curr_scan_topic;
  if (!sdf->HasElement("ros_topic")) {
    ROS_INFO_NAMED("livox_plugin", "livox plugin missing <ros_topic>, defaults to livox/scan");
    curr_scan_topic = "livox/scan";
  } else {
    curr_scan_topic = sdf->Get<std::string>("ros_topic");
  }

  ROS_INFO_STREAM("ros topic name:" << curr_scan_topic);
  ROS_INFO_STREAM("ros lidar frame id: " << lidar_frame_name_);
  ROS_INFO_STREAM("ros sensor frame id: " << sensor_frame_name_);
  ROS_INFO_STREAM("ros parent frame id: " << parent_frame_name_);
  ...
  switch (publishPointCloudType) {
    ...
    case livox_laser_simulation_CUSTOM_MSG:
      rosPointPub = nh_.advertise<mrs_gazebo_common_resources::LivoxCustomMsg>(curr_scan_topic, 5);
      break;
    default:
      break;
  }
...
void LivoxPointsPlugin::createStaticTransforms() {
  ...
  static_transform_sensor_base.header.frame_id         = parent_frame_name_;
  static_transform_sensor_base.child_frame_id          = sensor_frame_name_;
  static_transform_sensor_base.transform.translation.x = sensor_x;
  static_transform_sensor_base.transform.translation.y = sensor_y;
  static_transform_sensor_base.transform.translation.z = sensor_z;
  quat.setRPY(sensor_roll, sensor_pitch, sensor_yaw);
  ...
  static_transform_lidar_sensor.header.frame_id         = sensor_frame_name_;
  static_transform_lidar_sensor.child_frame_id          = lidar_frame_name_;
  static_transform_lidar_sensor.transform.translation.x = lidar_x;
  static_transform_lidar_sensor.transform.translation.y = lidar_y;
  static_transform_lidar_sensor.transform.translation.z = lidar_z;
  quat.setRPY(lidar_roll, lidar_pitch, lidar_yaw);
...
void LivoxPointsPlugin::InitializeRays(std::vector<std::pair<int, AviaRotateInfo>> &points_pair, boost::shared_ptr<physics::LivoxOdeMultiRayShape> &ray_shape) {
...
void LivoxPointsPlugin::PublishLivoxROSDriverCustomMsg(std::vector<std::pair<int, AviaRotateInfo>> &points_pair) {
  ...
  mrs_gazebo_common_resources::LivoxCustomMsg msg;
  ...
  for (int i = 0; i < points_pair.size(); ++i) {
    ...
    if (verticle_index < verticalRayCount && horizon_index < rayCount) {
      ...
      auto point = range * axis;
      // pt.intensity = static_cast<float>(intensity);
      mrs_gazebo_common_resources::LivoxCustomPoint pt;
      pt.x    = point.X();
      pt.y    = point.Y();
      pt.z    = point.Z();
      pt.line = pair.second.line;
